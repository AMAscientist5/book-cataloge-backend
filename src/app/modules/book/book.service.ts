...
// Create new book
const createNewBook = async (book_data: Book): Promise<Book | null> => {
...
//  Books By CategoryId
const gelBooksByCategoryId = async (
  ct_id: string,
  pagination_data: Partial<IPagination>
): Promise<GenericResponse<Book[]> | null> => {
  const { page, size, skip, sortObject } = pagination_map(
...
//book detail
const getBookDetails = async (id: string): Promise<Book | null> => {
...
//  Delete book
const deleteBook = async (book_id: string): Promise<Book | null> => {
  const isExist = await prisma.book.findUnique({ where: { id: book_id } });
