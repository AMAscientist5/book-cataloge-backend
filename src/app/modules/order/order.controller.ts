...
const AllOrdersList = catchAsync(async (req: Request, res: Response) => {
  ...
  if (user_details?.role === UserRole.customer) {
    result = await OrderServices.customerAllOrdersList(user_details);
  } else {
    result = await OrderServices.allOrdersList();
  }
...
const orderDetails = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await OrderServices.orderDetails(order_id, user_details);
...
const orderCreate = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await OrderServices.createOrder(orderedBooks, user_data);
